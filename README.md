...
# octoPus
Check out *octoPus*' presentation video!!!

<video width="600" controls>
  <source src="https://gitlab.com/simone.bregaglio/octopus/-/blob/master/video/octoPus_oristano.mp4" type="video/mp4">
...
<img src="codeStructure.png" align="left" width="250" height="125"> 

The console application is structured in multiple folders, each containing either input files or classes and processes serving specific purposes. The **Files** folder contains the input files that are needed to run the model: weather data and two parameter files one used to define the BBCH phases of grapevine (hostSusceptibilityParameters.csv) and the other to parameterize the eight primary infection models (octoPusParameters.csv). The **LLM** folder contains the LLama large language model and will have to be created by the user after cloning the repository (the LLM is not provided in the git, see below for more details on how to set it up). **ModelCaller** contains the call to Llama, to R.NET to un the random forest algorithm, and the loop to iterate through the weather data. **Models** stores the Datatype classes and the calls to both the eight epidemiology and the phenology models. **Readers** stores the pipelines to read input parametes, **octoPus.json** is the configuration file, **icon.ico** contains the console icon, and **Program.cs** is the entry point main method. 

This document will guide users thrpugh the steps needed to successfully run *octoPus* on their machines. First, the required packages and extensions needed to run the console app will be expained in detail in section [2. Required packages and extensions](#2-required-packages-and-extensions). Then, the inputs required and their structure will be analysed together with the user customizations in section [3. Inputs and customization3](#3-inputs-and-customization).
...
<div align="center">
<img src="WeatherDataTemplate.png" width="600" height="300"> 
</div>
...
At the moment, in v1.0, only hourly data are supported in the console application but later releases will also include routines to estimate hourly data from daily maximum and minimum temperatures. Multiple weather files can be used to perform simultaneous analysis on different locations by simply dropping them in the dedicated folder but some steps are required in the [3.4. JSON configuration file](#34-json-configuration-file) to properly use them.
...
<div align="center">
<img src="hostSusceptibility.png" width="150" height="75"> 
</div>
...
<div align="center">
<img src="octoPusParameters.png" width="600" height="300"> 
</div>
...
<div align="center">
<img src="octopus_1.png" width="250" height="125"> 
</div>
